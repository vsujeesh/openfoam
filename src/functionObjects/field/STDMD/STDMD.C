...
void Foam::functionObjects::STDMD::calcAp()
{
    ...
    if (Pstream::parRun())
    {
        ...
        if (Pstream::master())
        {
            ...
            // Compute the parallel direct tall skinny QR decomposition
            QRMatrix<RMatrix> QRM
            (
                Rx,
                QRMatrix<RMatrix>::outputTypes::REDUCED_R,
            ...
            // Compute Moore-Penrose pseudo-inverse of Rx
            RxInv_ = pinv(Rx);

            // Compute Gx
            const RMatrix Gx(Rx*(Gz_^Rx));

            // Compute Moore-Penrose pseudo-inverse of Gx
            const RMatrix GxInv(pinv(Gx));

            // Compute the convenience object A1
            A1 = RxInv_*GxInv;
        }
        Pstream::scatter(RxInv_);
        Pstream::scatter(A1);

        // Compute the convenience object A2
        SMatrix A2(QzUH_ & QzUH_);
        reduce(A2, sumOp<SMatrix>());

        // Compute the convenience object A3
        SMatrix A3(QzUH_ & QzLH_);
        reduce(A3, sumOp<SMatrix>());

        // Compute Ap by optimized matrix chain multiplication
...
{
    Log<< tab << "# " << name() << ": Computing eigen decomposition #" << endl;

    // Compute eigenvalues, and clip eigenvalues with values < minMagEVal_
    if (Pstream::master())
    {
        // Replace Ap by eigenvalues (in-place eigen decomposition)
...
void Foam::functionObjects::STDMD::calcMags()
{
    ...
    if (Pstream::master())
    {
        ...
        if (modeSorter_ == modeSorterType::FIRST_SNAPSHOT)
        {
            //- Method of first snapshot (i.e. Mode amplitude method)
            std::transform
            (
                oAmps_.cbegin(),
                oAmps_.cend(),
        ...
        else
        {
            ...
            {
                oMags_[i] = calcSorter(amp, oEVals_[i]);
                ++i;
            }
        }
    }
...
Foam::scalar Foam::functionObjects::STDMD::calcSorter
(
...
{
    Log<< tab << "# " << name() << ": Sorting modes #" << endl;

    switch (modeSorter_)
    {
        case modeSorterType::KIEWAT:
        {
            return sorterKiewat(amplitude, EVal, modeNorm, param);
        }

        case modeSorterType::KOU_ZHANG:
        {
            const List<scalar> weight(currIndex_, 1.0);
            return sorterKouZhang(weight, amplitude, EVal, modeNorm);
        }
...
Foam::scalar Foam::functionObjects::STDMD::sorterKouZhang
(
...
)
{
    ...
    #ifdef FULLDEBUG
    if (Foam::sqrt(GREAT) < mag(eval))
    {
        WarningInFunction
            << "  # Evaluation of pow(eval, i) will likely lead to"
            << " an overflow floating point exception due to the"
            << " high magnitude of eval = " << mag(eval) << "."
            << endl;
    }
    #endif
...
Foam::scalar Foam::functionObjects::STDMD::sorterKiewat
(
    const complex& amplitude,
    const complex& eval,
    const scalar modeNorm,
...
{
    List<scalar> weight(currIndex_);

    std::iota(weight.begin(), weight.end(), 1);

    weight = sin(twoPi/currIndex_*(weight - 1 - 0.25*currIndex_))*param + param;
...
void Foam::functionObjects::STDMD::calcOutput()
{
    ...
    {
        autoPtr<OFstream> osPtr =
            createFile("STDMD" + fieldName_, mesh_.time().value());
        OFstream& os = osPtr.ref();
...
:
    ...
    iMags_()
{
    Info<< "    ### This STDMD release is the beta release ###" << nl
